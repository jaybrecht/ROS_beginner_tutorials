...
int main(int argc, char **argv)
{
  ...
  if (argc < 2)
  {
    ROS_INFO_STREAM("usage: change_greeting_client <New Greeting>");
    return 1;
  }
  ...
  ros::NodeHandle n;
  ros::ServiceClient client = n.serviceClient<beginner_tutorials::ChangeGreeting>("change_greeting");
  beginner_tutorials::ChangeGreeting srv;
  srv.request.new_greeting = new_greeting;
  if (client.call(srv))
  {
    ROS_INFO_STREAM("The greeting has been changed from " << "\"" << srv.response.old_greeting
